...
contract Custodian {
    ...
    modifier onlySigner {
        require(signerSet[msg.sender] || primary == msg.sender);
        _;
    }
    ...
    function requestUnlock(
        ...
        address _callbackAddress,
        bytes4 _callbackSelector
    ) public onlySigner returns (bytes32 requestMsgHash)
    {
        ...
        uint256 nonce = uint256(keccak256(abi.encodePacked(block.number - 1, address(this), requestIdx)));

        requestMsgHash = keccak256(abi.encodePacked(nonce, uint256(0xFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFF)));
        ...
        emit Requested(_lockId, _callbackAddress, _callbackSelector, nonce, requestMsgHash, timeLockExpiry);
    }
    ...
    {
        Request memory request = requestMap[_requestMsgHash];

        // copy memory to locals before `delete`
