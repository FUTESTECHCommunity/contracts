...
/**
 * The Controller is used to manage whitelisting of contracts and and halt the normal use of Futurer’s contracts (e.g., if there is a vulnerability found in Futurer).  There is only one instance of the Controller, and it gets uploaded to the blockchain before all of the other contracts.  The `owner` attribute of the Controller is set to the address that called the constructor of the Controller.  The Futurer team can then call functions from this address to interact with the Controller.
 *
 */
